...
let都说没有变量提升，他其实有，但是存在暂时性死区的问题，导致它似乎没有变量提升。
...
在let赋值之前，let不可读写，这就是暂时性死区问题,导致let看似不存在变量提升，实则存在
...
const 定义的变量指针指向的地址不可以变化，指向地址的内容可以变化。这就解释了为什么const定义的对象修改属性不报错，因为js中对象是引用类型，引用地址是存放在栈内存中的，引用的内容是存放在堆内存中的，所以当你修改对象的属性值时并没有改变引用地址所以不会报错。
但是当你改变对象引用地址是就会出现错误，而基本类型是直接存放在栈内存中的，所以也不能直接修改。
